...
class count(Generic[_N]):
    @overload
    def __new__(cls) -> count[int]: no_effects()
    @overload
...
class repeat(Generic[_T]):
    @overload
    def __new__(cls, object: _T) -> Self: no_effects()
    @overload
...
class chain(Generic[_T]):
    def __new__(cls, *iterables: Iterable[_T]) -> Self: no_effects()
    def __next__(self) -> _T: ...
    def __iter__(self) -> Self: no_effects()
    @classmethod
    # We use type[Any] and not type[_S] to not lose the type inference from __iterable
    def from_iterable(cls: type[Any], iterable: Iterable[Iterable[_S]], /) -> chain[_S]: no_effects()
...
if sys.version_info >= (3, 10):
    class pairwise(Generic[_T_co]):
        def __new__(cls, iterable: Iterable[_T], /) -> pairwise[tuple[_T, _T]]: no_effects()
        def __iter__(self) -> Self: no_effects()
        def __next__(self) -> _T_co: ...
