...
class _Database(MutableMapping[bytes, bytes]):
    def __init__(self, path: StrOrBytesPath, /, *, flag: Literal["r", "w", "c", "n"], mode: int) -> None: unsafe()
    def __len__(self) -> int: unsafe()
    def __getitem__(self, key: _SqliteData) -> bytes: unsafe()
    def __setitem__(self, key: _SqliteData, value: _SqliteData) -> None: unsafe()
    def __delitem__(self, key: _SqliteData) -> None: unsafe()
    def __iter__(self) -> Generator[bytes]: unsafe()
    def close(self) -> None: unsafe()
    def keys(self) -> list[bytes]: unsafe()  # type: ignore[override]
    def __enter__(self) -> Self: no_effects()
    def __exit__(self, *args: Unused) -> None: unsafe()

def open(filename: StrOrBytesPath, /, flag: Literal["r", "w,", "c", "n"] = "r", mode: int = 0o666) -> _Database: unsafe()
