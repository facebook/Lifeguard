...
class HTTPMessage(email.message.Message[str, str]):
    def getallmatchingheaders(self, name: str) -> list[str]: no_effects()  # undocumented

@overload
def parse_headers(fp: SupportsReadline[bytes], _class: Callable[[], _MessageT]) -> _MessageT: no_effects()
@overload
...
class HTTPResponse(io.BufferedIOBase, BinaryIO):  # type: ignore[misc]  # incompatible method definitions in the base classes
    ...
    url: str
    def __init__(self, sock: socket, debuglevel: int = 0, method: str | None = None, url: str | None = None) -> None: no_effects()
    def peek(self, n: int = -1) -> bytes: unsafe()
    def read(self, amt: int | None = None) -> bytes: unsafe()
    def read1(self, n: int = -1) -> bytes: unsafe()
    def readinto(self, b: WriteableBuffer) -> int: unsafe()
    def readline(self, limit: int = -1) -> bytes: unsafe()  # type: ignore[override]
    @overload
    def getheader(self, name: str) -> str | None: no_effects()
    @overload
    def getheader(self, name: str, default: _T) -> str | _T: ...
    def getheaders(self) -> list[tuple[str, str]]: no_effects()
    def isclosed(self) -> bool: no_effects()
    def __iter__(self) -> Iterator[bytes]: ...
    def __enter__(self) -> Self: ...
    def __exit__(
        self, exc_type: type[BaseException] | None, exc_val: BaseException | None, exc_tb: types.TracebackType | None
    ) -> None: ...
    def info(self) -> email.message.Message: no_effects()
    def geturl(self) -> str: no_effects()
    def getcode(self) -> int: no_effects()
    def begin(self) -> None: unsafe()
...
class HTTPConnection:
    ...
    def getresponse(self) -> HTTPResponse: unsafe()
    def set_debuglevel(self, level: int) -> None: mutation()
    if sys.version_info >= (3, 12):
        def get_proxy_response_headers(self) -> HTTPMessage | None: no_effects()

    def set_tunnel(self, host: str, port: int | None = None, headers: Mapping[str, str] | None = None) -> None: mutation()
    def connect(self) -> None: unsafe()
    def close(self) -> None: unsafe()
    def putrequest(self, method: str, url: str, skip_host: bool = False, skip_accept_encoding: bool = False) -> None: unsafe()
    def putheader(self, header: str | bytes, *values: _HeaderValue) -> None: unsafe()
    def endheaders(self, message_body: _DataType | None = None, *, encode_chunked: bool = False) -> None: unsafe()
    def send(self, data: _DataType | str) -> None: unsafe()
...
class UnknownProtocol(HTTPException):
    def __init__(self, version: str) -> None: no_effects()

class UnknownTransferEncoding(HTTPException): ...
class UnimplementedFileMode(HTTPException): ...

class IncompleteRead(HTTPException):
    def __init__(self, partial: bytes, expected: int | None = None) -> None: no_effects()
    partial: bytes
    expected: int | None
...
class BadStatusLine(HTTPException):
    def __init__(self, line: str) -> None: no_effects()

class LineTooLong(HTTPException):
    def __init__(self, line_type: str) -> None: no_effects()
