...
pub struct ParsedModule {
    ...
    pub is_init: bool,
    /// Whether the source file is generated Thrift codegen, i.e. its name ends with one
...
/// Module suffixes the Thrift Python generators emit as pure declarations: every `.py`
/// in `thrift/python.bzl`'s emission list, plus thrift-py-deprecated's `.ttypes`.
/// The py3 `containers_FBTHRIFT_ONLY_DO_NOT_USE` is deliberately absent: it imports its
/// own parent `.types` module and documents that importing it out of order raises.
const THRIFT_GENERATED_SUFFIXES: &[&str] = &[
    ".ttypes",
    ".thrift_types",
    ...
    ".thrift_reflection",
    ".thrift_clients",
    ".thrift_mutable_clients",
    ".thrift_services",
    ".thrift_mutable_services",
    ".thrift_services_reflection",
];
...
mod tests {
    ...
    fn thrift_generated_detection() {
        ...
        for suffix in THRIFT_GENERATED_SUFFIXES {
            assert!(is_gen(&format!("pkg.foo{suffix}"), header), "{suffix}");
        }
        // py3 container glue imports its own parent and warns about import order.
        assert!(!is_gen(
            "pkg.foo.containers_FBTHRIFT_ONLY_DO_NOT_USE",
            header
        ));
